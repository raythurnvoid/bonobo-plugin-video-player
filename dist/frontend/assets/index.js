...
function read_bridge_bootstrap() {
  ...
  if (!fragment)
    throw new Error(
      "Missing host bridge fragment — the page must be embedded by the Bonobo host app",
    );
  ...
  if (
    params.size !== 2 ||
    parentOrigins.length !== 1 ||
    bridgeNonces.length !== 1
  )
...
async function bonobo_ui_connect() {
  ...
  function refreshToken() {
    ...
    refresh_in_flight = new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(
          /* @__PURE__ */ new Error("Plugin page token refresh timed out"),
        );
      }, REFRESH_DEADLINE_MS);
  ...
  async function fetchJson(path, init) {
    ...
    if (!response.ok) {
      ...
      throw Object.assign(
        /* @__PURE__ */ new Error(
          `${path} responded ${response.status}: ${responseText}`,
        ),
        {
 ...
 * LICENSE file in the root directory of this source tree.
 */
var require_react_dom_client_production = /* @__PURE__ */ __commonJSMin(
  (exports) => {
              ...
              return (
                ...
                value === noopSuspenseyCommitThenable
                  ...
                  : ((returnFiber = sourceFiber.updateQueue),
                    null === returnFiber
                      ? (sourceFiber.updateQueue = /* @__PURE__ */ new Set([
                          value,
                        ]))
...
function create_media_url_manager(client, nodeId) {
  ...
  function request_download_url() {
    ...
    const request = (async () => {
      try {
        const response = await fetch_json_with_429_retry(
          client,
