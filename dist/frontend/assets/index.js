...
//#endregion
//#region node_modules/.pnpm/convex@1.45.0_react@19.2.8/node_modules/convex/dist/esm/react/client.js
...
//#endregion
//#region node_modules/.pnpm/bonobo-plugin-sdk@https+++c_66ab31fe3df9b8431ea9c93aeeccbea7/node_modules/bonobo-plugin-sdk/frontend.js
/**
...
async function bonobo_connect() {
	...
	async function fetchJson(path, body, init) {
		...
		/** @param {string} reason */
		const refuse = (reason) =>
			Object.assign(/* @__PURE__ */ new Error(`${path} responded ${response.status}: ${reason}`), {
				status: response.status,
				responseText,
			});
		if (response.status >= 500) throw refuse(responseText);
		let parsedBody;
		try {
...
var require_react_dom_client_production = /* @__PURE__ */ __commonJSMin((exports) => {
	...
	function getNextLanes(root, wipLanes, rootHasPendingCommit) {
		...
		return 0 === nextLanes
			...
			: 0 !== wipLanes &&
				  ...
				  suspendedLanes >= rootHasPendingCommit || (32 === suspendedLanes && 0 !== (rootHasPendingCommit & 4194048)))
				? wipLanes
				: nextLanes;
	}
	...
	function updateInput(element, value, defaultValue, lastDefaultValue, checked, defaultChecked, type, name) {
		element.name = "";
		null != type && "function" !== typeof type && "symbol" !== typeof type && "boolean" !== typeof type
			? (element.type = type)
		...
		null != name && "function" !== typeof name && "symbol" !== typeof name && "boolean" !== typeof name
	...
	function updateTextarea(element, value, defaultValue) {
		if (
			null != value &&
			((value = "" + getToStringValue(value)), value !== element.value && (element.value = value), null == defaultValue)
		) {
			...
			a: switch (((target = internalInstance.stateNode), internalInstance.type)) {
				...
				case "select":
					((internalInstance = props.value),
						null != internalInstance && updateOptions(target, !!props.multiple, internalInstance, !1));
			}
		}
	}
		...
		a: switch (registrationName) {
			...
			case "onMouseEnter":
				(props = !props.disabled) ||
					((inst = inst.type),
					(props = !("button" === inst || "input" === inst || "select" === inst || "textarea" === inst)));
				inst = !props;
				break a;
			default:
				inst = !1;
	...
	function getEventCharCode(nativeEvent) {
		...
		"charCode" in nativeEvent
			? ((nativeEvent = nativeEvent.charCode), 0 === nativeEvent && 13 === keyCode && (nativeEvent = 13))
			: (nativeEvent = keyCode);
	...
	var useFallbackCompositionData =
		canUseDOM && (!canUseCompositionEvent || (documentMode && 8 < documentMode && 11 >= documentMode));
	...
	function getFallbackBeforeInputChars(domEventName, nativeEvent) {
		if (isComposing)
			return "compositionend" === domEventName ||
				...
				? ((domEventName = getData()), (fallbackText = startText = root = null), (isComposing = !1), domEventName)
				: null;
		switch (domEventName) {
			case "paste":
				return null;
	...
	function getActiveElementDeep(containerInfo) {
		containerInfo =
			null != containerInfo && null != containerInfo.ownerDocument && null != containerInfo.ownerDocument.defaultView
	...
	function finishQueueingConcurrentUpdates() {
		for (
			var endIndex = concurrentQueuesIndex, i = (concurrentlyUpdatedLanes = concurrentQueuesIndex = 0);
			i < endIndex;
		) {
	...
	function markUpdateLaneFromFiberToRoot(sourceFiber, update, lane) {
		...
		for (var isHidden = !1, parent = sourceFiber.return; null !== parent; )
			((parent.childLanes |= lane),
				...
				22 === parent.tag &&
					((sourceFiber = parent.stateNode), null === sourceFiber || sourceFiber._visibility & 1 || (isHidden = !0)),
				(sourceFiber = parent),
	...
	function createWorkInProgress(current, pendingProps) {
		...
		null === workInProgress
			? ((workInProgress = createFiberImplClass(current.tag, pendingProps, current.key, current.mode)),
		...
		for (var parent = workInProgress, isInsidePropagationBailout = !1; null !== parent; ) {
			...
			if (10 === parent.tag) {
				...
				currentParent.memoizedState.memoizedState !== parent.memoizedState.memoizedState &&
					(null !== current ? current.push(HostTransitionContext) : (current = [HostTransitionContext]));
			}
			parent = parent.return;
		}
		...
		function updateFromMap(existingChildren, returnFiber, newIdx, newChild, lanes) {
			...
			if ("object" === typeof newChild && null !== newChild) {
				switch (newChild.$$typeof) {
					case REACT_ELEMENT_TYPE:
						return (
							(existingChildren = existingChildren.get(null === newChild.key ? newIdx : newChild.key) || null),
			...
			do {
				...
				if (
					...
					null !== updateLane &&
						...
						null === isHiddenUpdate ? (queue.callbacks = [updateLane]) : isHiddenUpdate.push(updateLane));
				} else
					((isHiddenUpdate = {
						lane: updateLane,
						tag: pendingQueue.tag,
	...
	function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderLanes) {
		...
		ReactSharedInternals.H =
			null === current || null === current.memoizedState ? HooksDispatcherOnMount : HooksDispatcherOnUpdate;
	...
	function finishRenderingHooks(current) {
		...
		null === current ||
			didReceiveUpdate ||
			((current = current.dependencies), null !== current && checkIfContextChanged(current) && (didReceiveUpdate = !0));
	}
	...
	function useThenable(thenable) {
		...
		null === (null === workInProgressHook ? index.memoizedState : workInProgressHook.next) &&
			...
			(ReactSharedInternals.H =
				null === index || null === index.memoizedState ? HooksDispatcherOnMount : HooksDispatcherOnUpdate));
		return thenable;
	}
	function use(usable) {
	...
	function updateReducerImpl(hook, current, reducer) {
		...
		if (null === baseQueue) hook.memoizedState = pendingQueue;
			...
			if (
				...
				((didReceiveUpdate = !0),
				didReadFromEntangledAsyncAction$60 && ((reducer = currentEntangledActionThenable), null !== reducer))
			)
				throw reducer;
						...
						if (nextHydratableInstance) {
							b: {
								...
								JSCompiler_inline_result$jscomp$0 =
									"F!" === inRootOrSingleton || "F" === inRootOrSingleton ? JSCompiler_inline_result$jscomp$0 : null;
							}
	...
	function updateActionStateImpl(stateHook, currentStateHook, action) {
		...
		action !== currentStateHook.memoizedState &&
			...
			pushSimpleEffect(9, { destroy: void 0 }, actionStateActionEffect.bind(null, actionQueue, action), null));
		...
		for (var provider = fiber.return; null !== provider; ) {
			switch (provider.tag) {
				...
				case 3:
					...
					null !== root$69 &&
						(scheduleUpdateOnFiber(root$69, provider, lane), entangleTransitions(root$69, provider, lane));
	...
	var HooksDispatcherOnMount = {
		...
		useId: function () {
			...
			if (isHydrating) {
				...
				JSCompiler_inline_result =
					(idWithLeadingBit & ~(1 << (32 - clz32(idWithLeadingBit) - 1))).toString(32) + JSCompiler_inline_result;
				...
				((JSCompiler_inline_result = globalClientIdCounter++),
					(identifierPrefix = "_" + identifierPrefix + "r_" + JSCompiler_inline_result.toString(32) + "_"));
		...
		if ("hidden" === nextProps.mode) {
			...
			if (0 !== (renderLanes & 536870912))
				((workInProgress.memoizedState = {
					...
					null !== current && pushTransition(workInProgress, null !== prevState ? prevState.cachePool : null),
	...
	function updateClassComponent(current, workInProgress, Component, nextProps, renderLanes) {
		...
		if (null === workInProgress.stateNode) {
			...
			contextType = Component.contextType;
			context.context =
				"object" === typeof contextType && null !== contextType ? readContext(contextType) : emptyContextObject;
			context.state = workInProgress.memoizedState;
			...
			"function" === typeof Component.getDerivedStateFromProps ||
				...
				("function" !== typeof context.UNSAFE_componentWillMount && "function" !== typeof context.componentWillMount) ||
			...
			contextType$jscomp$0 =
				"function" === typeof getDerivedStateFromProps || "function" === typeof context.getSnapshotBeforeUpdate;
			...
			unresolvedOldProps || oldState !== oldContext || hasForceUpdate
				? ("function" === typeof getDerivedStateFromProps &&
					...
					(nextProps = oldProps))
				: ("function" === typeof context.componentDidMount && (workInProgress.flags |= 4194308), (nextProps = !1));
		} else {
			context = workInProgress.stateNode;
			...
			(oldContext =
				"function" === typeof unresolvedOldProps || "function" === typeof context.getSnapshotBeforeUpdate) ||
				("function" !== typeof context.UNSAFE_componentWillReceiveProps &&
					"function" !== typeof context.componentWillReceiveProps) ||
			...
			oldState !== newState ||
			hasForceUpdate ||
			(null !== current && null !== current.dependencies && checkIfContextChanged(current.dependencies))
		...
		context || nextProps
			? ((context = workInProgress.stateNode),
				(Component = nextProps && "function" !== typeof Component.getDerivedStateFromError ? null : context.render()),
				(workInProgress.flags |= 1),
	...
	function updateSuspenseComponent(current, workInProgress, renderLanes) {
		...
		(JSCompiler_temp = didSuspend) ||
			(JSCompiler_temp =
				null !== current && null === current.memoizedState ? !1 : 0 !== (suspenseStackCursor.current & 2));
		...
		if (null !== prevState && ((nextPrimaryChildren = prevState.dehydrated), null !== nextPrimaryChildren)) {
			if (didSuspend)
				workInProgress.flags & 256
					...
					: null !== workInProgress.memoizedState
						...
						: (reuseSuspenseHandlerOnStack(workInProgress),
							...
							(nextPrimaryChildren = createFiberFromFragment(nextPrimaryChildren, showFallback, renderLanes, null)),
				...
				isSuspenseInstancePending(nextPrimaryChildren)
					? ((workInProgress.flags |= 192), (workInProgress.child = current.child), (workInProgress = null))
					: ((current = prevState.treeContext),
		...
		switch (workInProgress.tag) {
					...
					if ("function" === typeof current)
						shouldConstruct(current)
							? ((props = resolveClassComponentProps(current, props)),
								...
								(workInProgress = updateClassComponent(null, workInProgress, current, props, renderLanes)))
							: ((workInProgress.tag = 0),
								(workInProgress = updateFunctionComponent(null, workInProgress, current, props, renderLanes)));
					else {
			...
			case 26:
				return (
					markRef(current, workInProgress),
					null === current
						? (renderLanes = getResource(workInProgress.type, null, workInProgress.pendingProps, null))
							? (workInProgress.memoizedState = renderLanes)
							: isHydrating ||
								((renderLanes = workInProgress.type),
								(current = workInProgress.pendingProps),
								(props = getOwnerDocumentFromRootContainer(rootInstanceStackCursor.current).createElement(renderLanes)),
			...
			case 5:
				...
				shouldSetTextContent($$typeof, prevState)
					? (props = null)
					: null !== nextState && shouldSetTextContent($$typeof, nextState) && (workInProgress.flags |= 32);
				null !== workInProgress.memoizedState &&
					(($$typeof = renderWithHooks(current, workInProgress, TransitionAwareHostComponent, null, null, renderLanes)),
			...
			case 6:
				if (null === current && isHydrating) {
					if ((current = renderLanes = nextHydratableInstance))
						((renderLanes = canHydrateTextInstance(renderLanes, workInProgress.pendingProps, rootOrSingletonContext)),
							null !== renderLanes
		...
		switch (workInProgress.tag) {
			...
			case 26:
				...
				null === current
					? (markUpdate(workInProgress),
						null !== nextResource
							? (bubbleProperties(workInProgress), preloadResourceAndSuspendIfNeeded(workInProgress, nextResource))
							: (bubbleProperties(workInProgress),
					...
					: nextResource
						...
						: ((current = current.memoizedProps),
							...
							bubbleProperties(workInProgress),
							preloadInstanceAndSuspendIfNeeded(workInProgress, type, current, newProps, renderLanes));
				return null;
			case 27:
				...
				if (null === current || (null !== current.memoizedState && null !== current.memoizedState.dehydrated)) {
					...
					if (null !== newProps && null !== newProps.dehydrated) {
						...
						((type = upgradeHydrationErrorsToRecoverable()),
							null !== current && null !== current.memoizedState && (current.memoizedState.hydrationErrors = type),
							(type = !0));
					if (!type) {
						if (workInProgress.flags & 256) return (popSuspenseHandler(workInProgress), workInProgress);
						popSuspenseHandler(workInProgress);
				...
				if (null === nextResource)
					...
					if (!type)
						if (((current = findFirstSuspended(nextResource)), null !== current)) {
							if (
								...
								null === newProps.tail && "hidden" === newProps.tailMode && !nextResource.alternate && !isHydrating)
							)
								return (bubbleProperties(workInProgress), null);
						} else
					...
					newProps.isBackwards
						...
						: ((current = newProps.last),
							null !== current ? (current.sibling = nextResource) : (workInProgress.child = nextResource),
				...
				return (
					...
					newProps
						? 0 !== (renderLanes & 536870912) &&
							...
							(bubbleProperties(workInProgress), workInProgress.subtreeFlags & 6 && (workInProgress.flags |= 8192))
						: bubbleProperties(workInProgress),
		...
		if (5 === tag || 6 === tag)
			((node = node.stateNode),
				before
					...
					: ((before =
							9 === parent.nodeType ? parent.body : "HTML" === parent.nodeName ? parent.ownerDocument.body : parent),
	...
	function insertOrAppendPlacementNode(node, before, parent) {
		...
		if (5 === tag || 6 === tag)
			...
			4 !== tag &&
			(27 === tag && isSingletonScope(node.type) && (parent = node.stateNode), (node = node.child), null !== node)
		)
	...
	function commitBeforeMutationEffects(root, firstChild) {
		...
		for (nextEffect = firstChild; null !== nextEffect; )
			if (
				((firstChild = nextEffect), (root = firstChild.child), 0 !== (firstChild.subtreeFlags & 1028) && null !== root)
			)
				...
				for (; null !== nextEffect; ) {
					...
					switch (firstChild.tag) {
						case 0:
							if (
								0 !== (root & 4) &&
								((root = firstChild.updateQueue), (root = null !== root ? root.events : null), null !== root)
							)
		...
		switch (finishedWork.tag) {
												...
												for (var i = 0; i < maybeNodes.length; i++)
													if (
														...
														currentResource.getAttribute("href") ===
															...
															currentResource.getAttribute("rel") === (null == current.rel ? null : current.rel) &&
												...
												for (i = 0; i < maybeNodes.length; i++)
													if (
														...
														currentResource.getAttribute("content") ===
															...
															currentResource.getAttribute("name") === (null == current.name ? null : current.name) &&
			...
			case 5:
				...
				flags & 4 &&
					...
					commitHostUpdate(finishedWork, hoistableRoot, null !== current ? current.memoizedProps : hoistableRoot));
			...
			case 31:
				...
				flags & 4 &&
					...
					null !== flags && ((finishedWork.updateQueue = null), attachSuspenseRetryListeners(finishedWork, flags)));
				break;
			case 13:
				recursivelyTraverseMutationEffects(root, finishedWork);
				commitReconciliationEffects(finishedWork);
				finishedWork.child.flags & 8192 &&
					(null !== finishedWork.memoizedState) !== (null !== current && null !== current.memoizedState) &&
			...
			case 22:
				...
				if (flags & 8192)
					a: for (
						...
						if (5 === root.tag || 26 === root.tag) {
								...
								try {
									if (((currentResource = wasHidden.stateNode), hoistableRoot))
										...
										var styleProp = wasHidden.memoizedProps.style,
											display =
												void 0 !== styleProp && null !== styleProp && styleProp.hasOwnProperty("display")
													? styleProp.display
													: null;
							...
							((22 !== root.tag && 23 !== root.tag) || null === root.memoizedState || root === finishedWork) &&
							null !== root.child
						) {
				...
				flags & 4 &&
					...
					null !== flags &&
						...
						null !== current && ((flags.retryQueue = null), attachSuspenseRetryListeners(finishedWork, current))));
				break;
			case 19:
				recursivelyTraverseMutationEffects(root, finishedWork);
				commitReconciliationEffects(finishedWork);
	...
	function commitPassiveMountOnFiber(finishedRoot, finishedWork, committedLanes, committedTransitions) {
		...
		switch (finishedWork.tag) {
			...
			case 3:
				...
				flags & 2048 &&
					((finishedRoot = null),
					null !== finishedWork.alternate && (finishedRoot = finishedWork.alternate.memoizedState.cache),
					(finishedWork = finishedWork.memoizedState.cache),
	...
	function scheduleUpdateOnFiber(root, fiber, lane) {
		if (
			(root === workInProgressRoot && (2 === workInProgressSuspendedReason || 9 === workInProgressSuspendedReason)) ||
			...
			: thrownValue === SuspenseyCommitException
				...
				: (workInProgressSuspendedReason =
						thrownValue === SelectiveHydrationException
							? 8
							: null !== thrownValue && "object" === typeof thrownValue && "function" === typeof thrownValue.then
								? 6
								: 1);
	...
	function throwAndUnwindWorkLoop(root, unitOfWork, thrownValue, suspendedReason) {
		...
		if (unitOfWork.flags & 32768) {
			if (isHydrating || 1 === suspendedReason) root = !0;
				...
				2 === suspendedReason || 9 === suspendedReason || 3 === suspendedReason || 6 === suspendedReason)
			)
								...
								if (win.getSelection) {
									var selection = win.getSelection(),
										...
										start$jscomp$0 = Math.min(priorSelectionRange.start, length),
											...
											void 0 === priorSelectionRange.end ? start$jscomp$0 : Math.min(priorSelectionRange.end, length);
									!selection.extend &&
	...
	function attachPingListener(root, wakeable, lanes) {
		...
		if (null === pingCache) {
			...
			((threadIDs = pingCache.get(wakeable)),
				void 0 === threadIDs && ((threadIDs = /* @__PURE__ */ new Set()), pingCache.set(wakeable, threadIDs)));
		threadIDs.has(lanes) ||
				...
				for (var root$170 = firstScheduledRoot; null !== root$170; ) {
					if (!onlyLegacy)
						if (0 !== syncTransitionLanes) {
							...
							0 !== JSCompiler_inline_result &&
								((didPerformSomeWork = !0), performSyncWorkOnRoot(root$170, JSCompiler_inline_result));
						} else
	...
	function scheduleTaskForRootDuringMicrotask(root, currentTime) {
		...
		if (
			0 === suspendedLanes ||
			(root === currentTime && (2 === workInProgressSuspendedReason || 9 === workInProgressSuspendedReason)) ||
			null !== root.cancelPendingCommit
		)
			return (
	...
	for (var i$jscomp$inline_1577 = 0; i$jscomp$inline_1577 < simpleEventPluginEvents.length; i$jscomp$inline_1577++) {
		...
		registerSimpleEvent(
			...
			"on" + (eventName$jscomp$inline_1578[0].toUpperCase() + eventName$jscomp$inline_1578.slice(1)),
		);
	}
	...
	function listenToNonDelegatedEvent(domEventName, targetElement) {
		...
		void 0 === JSCompiler_inline_result &&
			(JSCompiler_inline_result = targetElement[internalEventHandlersKey] = /* @__PURE__ */ new Set());
	...
	function listenToAllSupportedEvents(rootContainerElement) {
		if (!rootContainerElement[listeningMarker]) {
			...
			var ownerDocument =
				9 === rootContainerElement.nodeType ? rootContainerElement : rootContainerElement.ownerDocument;
			null === ownerDocument ||
				...
				if (void 0 !== reactName) {
					...
					for (var instance = targetInst, lastHostComponent; null !== instance; ) {
						...
						(5 !== _instance && 26 !== _instance && 27 !== _instance) ||
							...
							null != _instance && inCapturePhase.push(createDispatchListener(instance, _instance, lastHostComponent)));
	...
	function accumulateTwoPhaseListeners(targetFiber, reactName) {
		for (var captureName = reactName + "Capture", listeners = []; null !== targetFiber; ) {
			...
			(5 !== _instance2 && 26 !== _instance2 && 27 !== _instance2) ||
				...
				null != _instance2 && listeners.unshift(createDispatchListener(targetFiber, _instance2, stateNode)),
				(_instance2 = getListener(targetFiber, reactName)),
				null != _instance2 && listeners.push(createDispatchListener(targetFiber, _instance2, stateNode)));
			...
			(5 !== _instance3 && 26 !== _instance3 && 27 !== _instance3) ||
				...
				inCapturePhase
					? ((stateNode = getListener(target, registrationName)),
						null != stateNode && listeners.unshift(createDispatchListener(target, stateNode, alternate)))
					: inCapturePhase ||
						((stateNode = getListener(target, registrationName)),
						null != stateNode && listeners.push(createDispatchListener(target, stateNode, alternate))));
	...
	function setProp(domElement, tag, key, value, props, prevValue) {
		switch (key) {
			...
			case "size":
			case "span":
				null != value && "function" !== typeof value && "symbol" !== typeof value && !isNaN(value) && 1 <= value
					? domElement.setAttribute(key, value)
					: domElement.removeAttribute(key);
				break;
			case "rowSpan":
	...
	function setPropOnCustomElement(domElement, tag, key, value, props, prevValue) {
		switch (key) {
			...
			case "children":
				"string" === typeof value
					? setTextContent(domElement, value)
					: ("number" === typeof value || "bigint" === typeof value) && setTextContent(domElement, "" + value);
				break;
			case "onScroll":
				null != value && listenToNonDelegatedEvent("scroll", domElement);
				break;
				...
				if (!registrationNameDependencies.hasOwnProperty(key))
					a: {
						if (
							...
							"function" === typeof prevValue && domElement.removeEventListener(tag, prevValue, props),
				...
				if (isCustomElement(tag)) {
					for (propValue$184 in props)
						props.hasOwnProperty(propValue$184) &&
							...
							void 0 !== hasSrc && setPropOnCustomElement(domElement, tag, propValue$184, hasSrc, props, void 0));
					return;
				}
		}
		for (defaultValue in props)
			props.hasOwnProperty(defaultValue) &&
				((hasSrc = props[defaultValue]), null != hasSrc && setProp(domElement, tag, defaultValue, hasSrc, props, null));
	}
			...
			case "textarea":
				...
				for (defaultValue in lastProps)
					if (
						...
						lastProps.hasOwnProperty(defaultValue) && null != name && !nextProps.hasOwnProperty(defaultValue))
					)
						switch (defaultValue) {
							case "value":
								break;
			...
			case "option":
				for (var propKey$217 in lastProps)
					if (
						...
						lastProps.hasOwnProperty(propKey$217) && null != propKey$201 && !nextProps.hasOwnProperty(propKey$217))
					)
						switch (propKey$217) {
							case "selected":
				...
				for (lastDefaultValue in nextProps)
					if (
						...
						switch (lastDefaultValue) {
							case "selected":
								domElement.selected =
									propKey$201 && "function" !== typeof propKey$201 && "symbol" !== typeof propKey$201;
								break;
							default:
			...
			case "menuitem":
				...
				for (checked in nextProps)
					if (
						...
						nextProps.hasOwnProperty(checked) && propKey$201 !== propKey && (null != propKey$201 || null != propKey))
					)
						switch (checked) {
							case "children":
							case "dangerouslySetInnerHTML":
					...
					for (var propKey$227 in lastProps)
						((propKey$201 = lastProps[propKey$227]),
							lastProps.hasOwnProperty(propKey$227) &&
								...
								setPropOnCustomElement(domElement, tag, propKey$227, void 0, nextProps, propKey$201));
					for (defaultChecked in nextProps)
						((propKey$201 = nextProps[defaultChecked]),
							...
							!nextProps.hasOwnProperty(defaultChecked) ||
								...
								setPropOnCustomElement(domElement, tag, defaultChecked, propKey$201, nextProps, propKey));
					return;
				}
		}
				...
				if (transferSize && duration && isLikelyStaticResource(initiatorType)) {
					...
					for (i += 1; i < resourceEntries.length; i++) {
						...
						overlapTransferSize &&
							...
							(initiatorType +=
								overlapTransferSize *
								(overlapEntry < duration ? 1 : (duration - overlapStartTime) / (overlapEntry - overlapStartTime))));
					}
					--i;
				...
				switch (type) {
					...
					case "link":
						...
						if ("stylesheet" === name && instance.hasAttribute("data-precedence")) break;
							...
							instance.getAttribute("crossorigin") !== (null == anyProps.crossOrigin ? null : anyProps.crossOrigin) ||
		...
		if (ownerDocument && href && as) {
			...
			"image" === as
				? options && options.imageSrcSet
					? ((preloadSelector +=
							'[imagesrcset="' + escapeSelectorAttributeValueInsideDoubleQuotes(options.imageSrcSet) + '"]'),
						"string" === typeof options.imageSizes &&
							(preloadSelector +=
								'[imagesizes="' + escapeSelectorAttributeValueInsideDoubleQuotes(options.imageSizes) + '"]'))
			...
			case "link":
				if (
					...
					resource$244 ||
						...
						(styles$243 = JSCompiler_inline_result.querySelector(getStylesheetSelectorFromKey(type))) &&
							!styles$243._p &&
						...
						preloadPropsMap.has(type) ||
							...
							styles$243 || preloadStylesheet(JSCompiler_inline_result, type, pendingProps, resource$244.state)));
			...
			? function (commit) {
					...
					var imgTimer = setTimeout(
						function () {
							...
							if (
								0 === state.count &&
								(state.stylesheets && insertSuspendedStylesheets(state, state.stylesheets), state.unsuspend)
							) {
								var unsuspend = state.unsuspend;
								state.unsuspend = null;
								unsuspend();
	...
	function insertStylesheetIntoRoot(root, resource) {
		if (!(resource.state.loading & 4)) {
			...
			i
				...
				: ((root = 9 === root.nodeType ? root.head : root), root.insertBefore(nodes, root.firstChild));
			resource.state.loading |= 4;
		}
	}
	...
	function updateContainerImpl(rootFiber, lane, element, container, parentComponent, callback) {
		...
		null !== element &&
			(scheduleUpdateOnFiber(element, rootFiber, lane), entangleTransitions(element, rootFiber, lane));
	}
	...
	function accumulateOrCreateContinuousQueuedReplayableEvent(
		...
		if (null === existingQueuedEvent || existingQueuedEvent.nativeEvent !== nativeEvent)
			return (
				...
				null !== blockedOn &&
					((blockedOn = getInstanceFromNode(blockedOn)), null !== blockedOn && attemptContinuousHydration(blockedOn)),
				existingQueuedEvent
			);
	...
	function retryIfBlockedOn(unblocked) {
		...
		for (
			;
			0 < queuedExplicitHydrationTargets.length && ((i = queuedExplicitHydrationTargets[0]), null === i.blockedOn);
		)
	...
	if ("19.2.8" !== isomorphicReactPackageVersion$jscomp$inline_1840)
		throw Error(formatProdErrorMessage(527, isomorphicReactPackageVersion$jscomp$inline_1840, "19.2.8"));
 ...
 *
 * Every other status comes back for the caller to narrow on. Only a 5xx, a body that is not JSON,
 * a refused session refresh, and a network failure still reject.
 */
async function fetch_json_with_429_retry(client, path, body) {
	for (let attempt = 0; ; attempt += 1) {
		...
		if (answer.status === 429 && delay_ms !== void 0) {
 ...
 */
function download_urls_body(answer) {
	if (answer.status !== 200) throw new Error(answer.body.message);
	return answer.body;
}
/**
 ...
 */
function create_media_url_manager(client, nodeId) {
	let cached = null;
	let in_flight = null;
	function request_download_url() {
...
function App(props) {
	...
	const request_url = (0, import_react.useCallback)(
		(fresh) => {
			(fresh ? media.get_fresh_url() : media.get_url()).then(
				(media_url) => {
	...
	const handle_video_loaded_metadata = (0, import_react.useCallback)(() => {
		autoRenewSpentRef.current = false;
		const video = videoRef.current;
		if (!video) return;
	...
	const handle_retry = (0, import_react.useCallback)(() => {
		autoRenewSpentRef.current = false;
		setErrorMessage(null);
		request_url(true);
	}, [request_url]);
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
						...
						children: props.context.file.name,
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "player-meta",
						children: props.context.file.path,
					}),
				],
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children:
					errorMessage !== null
						? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "player-status is-error",
								role: "alert",
								children: [
		...
		if (context.kind !== "file_view") {
			root.render(
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(BootScreen, {
					message: "This plugin only provides a file view",
					isError: true,
				}),
			);
			return;
